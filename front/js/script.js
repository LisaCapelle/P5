...
// function to display the whole list of products on index.html page
            ...
            for (let x = 0; x < numberOfProducts; x++) {
                ...
                items.innerHTML += `<a href="./product.html?id=${productList[x]._id}">
                <article>
                <img src="${productList[x].imageUrl}" alt="${productList[x].altTxt}">
                <h3 class="productName">${productList[x].name}</h3>
                <p class="productDescription">${productList[x].description}</p>
                </article>
                </a>`
            }
        })
